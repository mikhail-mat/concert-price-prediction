...
After completing this project, I realised I fit my encoding transformations
on the full dataset before splitting into train and test sets. This means
information from the test set influenced training, which is a form of data leakage.
The correct approach is to split first, then fit transformations only on
the training data and apply them to the test set. In future projects I make
sure all preprocessing happens inside an sklearn pipeline fit only on
training data.
